...
function [Rplain,Rortho] = OrthogonalPowCorr(BPCD,noGPU)
...
%  Determine if a capable GPU is present to perform the orthogonalization.
toolboxInfo = ver;
useGPU = false;
...
if any(strcmp({toolboxInfo.Name},'Parallel Computing Toolbox'))
    if gpuDeviceCount > 0
        gpu = gpuDevice(1);
        SSinfo = whos('SS');
        RorthoABinfo = whos('RA');
        % Check to make sure the detected GPU has enough available memory.
        if gpu.AvailableMemory >= 2 * SSinfo.bytes + RorthoABinfo.bytes
            useGPU = true;
        else
            warning('A GPU was detected, but it does not have sufficient memory.Using the CPU to perform the orthogonalization...')
        end
    else
        warning(['No GPU was detected, using the CPU to perform the orthogonalization...' ...
            'This will take substantially longer computation time (possibly a couple of hours or more).'])
    end
else
    warning(['No Parallel Computing Toolbox was detected (required to use GPUs)...' ...
        'Using the CPU to perform the orthogonalization...' ...
        ' This will take substantially longer computation time (possibly a couple of hours or more).'])
end
...
for v=1:nVerts
  ...
  if v > 500
      if mod(v,100) == 0
         waitbar(v/size(SS,1),waitbarHandle,sprintf('Calculating orthogonalized connectivity.\nVertex %u of %u complete.',v,size(SS,1)));
      end
  end
end
